...
public:
    ...
    // 历史所有关键帧（已降采样）的角点、平面点集合
    vector<pcl::PointCloud<PointType>::Ptr> cornerCloudKeyFrames;
    vector<pcl::PointCloud<PointType>::Ptr> surfCloudKeyFrames;

    // 历史关键帧优化后的位置,X Y Z I（i表示关键帧index）
    pcl::PointCloud<PointType>::Ptr cloudKeyPoses3D;
    // 历史关键帧优化后的位置,XYZI、RPY、time，始终与3D保持同步
    ...
    // 当前激光帧角点、平面点集合，降采样，DS: DownSize
    pcl::PointCloud<PointType>::Ptr laserCloudCornerLastDS; // downsampled corner featuer set from odoOptimization
    pcl::PointCloud<PointType>::Ptr laserCloudSurfLastDS;   // downsampled surf featuer set from odoOptimization
    // 当前帧与局部map匹配上了的角点、平面点，加入同一集合；coeffSel是对应点的参数
    ...
    // 依次为 R P Y ,X Y Z，表示激光所在位置，初始估计和优化都会对其进行更新
    float transformTobeMapped[6];

    std::mutex mtx; // 点云相关操作的互斥量
    std::mutex mtxLoopInfo;
    // 该值反映了在第一次迭代时，该方向特征值超过了阈值
    ...
    */
    mapOptimization()
    {
        ...
        //TODO 发布激光里程计，它与上面的激光里程计基本一样，只是roll、pitch用imu数据加权平均了一下，z做了限制（没看到）
    ...
    */
    void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr &msgIn)
    {
        ...
        // mapping执行频率控制，默认0.15s
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval)
        {
            timeLastProcessing = timeLaserInfoCur;
            // 当前帧位姿初始化。并不是基于匀速运动假设，而是用imu的增量进行初始化。
            // 1、如果是第一帧，或者是imu里程计出问题了，则用原始imu数据的RPY初始化当前帧位姿（仅旋转部分，感觉没必要舍弃位移）
            // 2、后续帧，用imu里程计计算两帧之间的增量位姿变换，作用于前一帧的激光位姿，得到当前帧激光位姿，存于transformTobeMapped
            updateInitialGuess();
            // 提取附近关键帧，并组成局部角-边点地图
            // 1. KD树提取上一个点云帧附近的关键帧位置，并进行降采样
            // 2. 添加早于当前帧10s内的关键帧
            ...
            // 当有GPS或闭环时，更新因子图中所有变量节点（历史关键帧）的位姿，更新里程计轨迹
            correctPoses();
            // 发布激光里程计
            publishOdometry();
            // 发布里程计、点云、轨迹
...
/**
 * @brief 根据6D，将点云转换至直接坐标系
 * 
 * @param cloudIn 原始点云
 * @param transformIn 6D点
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    bool saveMapService(lio_sam::save_mapRequest &req, lio_sam::save_mapResponse &res)
    {
        ...
        // create directory and remove old files; 为了不删除文件夹，故注释
...
/**
 * @brief 搜索key附近searchNum帧点云，将其特征点组合成局部特征地图。
 * 
 * @param nearKeyframes out 拼接的局部特征地图
 * @param key 点云序号
 * @param searchNum 局部地图大小
 */
    void loopFindNearKeyframes(pcl::PointCloud<PointType>::Ptr &nearKeyframes, const int &key, const int &searchNum)
    {
        ...
        nearKeyframes->clear();
        int cloudSize = copy_cloudKeyPoses6D->size();
        // 若searchNum为0则跳过for，否则将前后若干帧的角点和面点都集合起来。
...
#pragma omp parallel for num_threads(numberOfCores)
        for (int i = 0; i < laserCloudSurfLastDSNum; i++)
        {
            ...
            if (pointSearchSqDis[4] < 1.0)
            {
                ...
                // 假设平面方程为ax+by+cz+d=0，这里就是求方程的系数abc，matB0=-d=-1
                //colPivHouseholderQr是求解matA0 x = matB0 这个方程式。
    ...
    {
        // 这个优化程序似乎是为camera写的，因此数据需要被转换到camere坐标系里。
        ...
        // yaw = pitch          ---     yaw = roll
        // srx crx分别表示 sin(roll)和cos(roll),下面类似
        // lidar -> camera
    ...
    void transformUpdate()
    {
        ...
        // 相当于对roll pitch z加了先验的饱和函数，作用不大
    ...
    void addGPSFactor()
    {
        ...
        // 位姿(x和y)协方差很小，没必要加入GPS数据进行校正
        // (3,3)和(4,4)每次update后更新，是isam对协方差边缘化的结果，可以表征当前位姿估计的置信度
        ...
        while (!gpsQueue.empty())
        {
            ...
            else
            {
                ...
                // gps高程估计比较差的话，在params里禁用，并用激光的z值去覆盖gps的z值。
                if (!useGpsElevation)
                {
    ...
    */
    ...
    {
        // 计算当前帧与前一帧位姿变换，如果变化足够大则返回true，表示需要设为关键帧
        // 如果没有关键帧则直接返回true
        if (saveFrame() == false)
            return;

        // odom factor
        // 第一帧则一个误差比较大的Prior Factor，其余则是between factor
        addOdomFactor();
    ...
    /**
     * 若有GPS或回环则清空局部地图，更新因子图中所有变量节点（历史关键帧）的位姿，更新里程计轨迹
    */
    void correctPoses()
    {
        if (cloudKeyPoses3D->points.empty())
    ...
    {
        // 发布激光里程计/mapping/odometry，值来自因子图优化
        ...
        {
            // Front是上一帧优化后的位姿，Back是当前帧激光里程计和IMU插值后的结果，
            Eigen::Affine3f affineIncre = incrementalOdometryAffineFront.inverse() * incrementalOdometryAffineBack;
            // increOdomAffine是上次优化到这次激光的变换的累积，相比上面的里程计，缺乏了这次激光到这次优化的变换
            // 从数据来看，前期是一样的，但后期有闭环引入后是不一样的。
            increOdomAffine = increOdomAffine * affineIncre;
            float x, y, z, roll, pitch, yaw;
            pcl::getTranslationAndEulerAngles(increOdomAffine, x, y, z, roll, pitch, yaw);
            if (cloudInfo.imuAvailable == true)
            {
                // imu姿态对结果进行再次加权，这里的权重是写死的，而不是从yaml拿，相当于这部分经过了两次插值
                if (std::abs(cloudInfo.imuPitchInit) < 1.4)
                {
    ...
    void publishFrames()
    {
        ...
        // Publish 局部地图的平面点，lio_sam/mapping/map_local
        publishCloud(&pubRecentKeyFrames, laserCloudSurfFromMapDS, timeLaserInfoStamp, odometryFrame);
        // publish当前关键帧匹配后的特征点，"lio_sam/mapping/cloud_registered"
